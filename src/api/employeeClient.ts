...
class EmployeeClient {
    ...
    async getListOfEmployeeNumbers(): Promise<number[]> {
        const employeeNumbers = this.EMPLOYEE_DATA.map((employee) => employee.employeeNumber);
        return employeeNumbers;
    }

    async updateEmployee(updatedEmp: Employee): Promise<void> {
        this.EMPLOYEE_DATA = this.EMPLOYEE_DATA.map((emp) => (
            emp.employeeNumber === updatedEmp.employeeNumber ?
                updatedEmp :
                emp
        ));
    }
    ...
    async deleteEmployee(employeeNumber: number): Promise<void> {
        this.EMPLOYEE_DATA = this.EMPLOYEE_DATA.filter((employee) => employee.employeeNumber !== employeeNumber);
    }
}
